...
export class SelectionComponent implements OnInit {
  ...
  dataFirstB = {
    label: "",
            data: this.selection(this.bestData),
            lineTension: 0.3,
            fill: false,
            borderColor: 'red',
  ...
  dataFirstM = {
    label: "",
            data: this.selection(this.mediumData),
            lineTension: 0.3,
            fill: false,
            borderColor: 'red',
            backgroundColor: 'transparent',
  ...
  dataFirstW = {
    label: "",
            data: this.selection(this.worstData),
            lineTension: 0.3,
            fill: false,
            borderColor: 'red',
            backgroundColor: 'transparent',
  ...
  ngOnInit() : void{
    this.chartBest = new Chart('bestS', {
      type: 'line',
      data: this.speedDataB,
      options: this.chartOptions
    });

    this.chartPromediate = new Chart('promediateS', {
      type: 'line',
      data: this.speedDataM,
      options: this.chartOptions
    });

    this.chartWorst = new Chart('worstS', {
      type: 'line',
  ...
  selection(data) : any {
    ...
    for( var slot = 0; slot < size -1; slot ++ ){ // outer loop
      ...
      for( var check = slot + 1; check < size; check++ ){ // inner loop
        if( data[check] < data[smallest] ){
          smallest = check;
        }
      }
      this.swapselection( data, smallest, slot );
    }
  ...
  swapselection(myArr, indexOne, indexTwo) : any{
